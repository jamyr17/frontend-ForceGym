        ...
        <div className="bg-black min-h-screen">
            <header className="flex ml-12 h-20 w-0.90 items-center text-black bg-yellow justify-between px-4">
                ...
                <SearchInput searchTerm={searchTerm} handleSearch={handleSearch} changeSearchType={changeSearchType} >
                    <option className="checked:bg-yellow hover:cursor-pointer hover:bg-slate-400" value={2} defaultChecked={searchType === 1}>Nombre</option>
                </SearchInput>
                                ...
                                {clientTypes?.map((clientType, index) => (
                                    <tr key={clientType.idClientType} className="text-center py-8">
                                        ...
                                        <td className="flex gap-4 justify-center py-2">
                                            ...
                                            {clientType.isDeleted ? (
                                                <button onClick={() => handleRestore(mapClientTypeToDataForm(clientType))} className="p-2 bg-black rounded-sm hover:bg-slate-700 hover:cursor-pointer">
                                                    <MdOutlineSettingsBackupRestore className="text-white" />
                                                </button>
                                            ) : (
                                                <button onClick={() => handleDelete(clientType)} className="p-2 bg-black rounded-sm hover:bg-gray-700 hover:cursor-pointer"
                                                    title="Eliminar">
